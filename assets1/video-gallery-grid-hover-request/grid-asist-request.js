  ...
  /* Ghost Plugin Code start âš ï¸ Copyright 2020 */	
  ...
  if (typeof(isGallery) != 'undefined' && isGallery != null){
    ...
    elementsArray.forEach(function(elem) {
      ...
      var overlayDescription = '<div class="custom-gallery-desc"><div class="inner-cust-wrapper"><h3>'+elemTitle+'</h3><div class="desc-wrapper">'+elemDesc+'</div></div></div>';
      ...
      elem.querySelectorAll('.desc-wrapper p a').forEach(function(thisNode){
        thisNode .addEventListener("click", function(e) {
          e.preventDefault();
          e.stopPropagation();
          if( thisNode.getAttribute('target') == "_blank"){
            var win = window.open(thisNode.getAttribute('target'), '_blank');
            win.focus();
          }else{
             document.location.href = thisNode.getAttribute('href')
          }
        });
      })
    });
